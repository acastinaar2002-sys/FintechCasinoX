    ...
    <div className="h-full overflow-y-auto bg-[#050505] p-6 md:p-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Panel de Control & Analítica en Tiempo Real</p>
        </div>
        <div className="flex items-center gap-3 bg-[#1a1d21] px-4 py-2 rounded-lg border border-white/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
        <div className="bg-[#141417] p-6 rounded-2xl border border-white/5">
           <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs text-slate-500 uppercase font-bold">Total Usuarios</span>
           </div>
        ...
        <div className="bg-[#141417] p-6 rounded-2xl border border-white/5">
           <div className="flex justify-between items-start mb-4">
              <div className="p-3 rounded-lg bg-[#D4C28A]/10 text-[#D4C28A]"><DollarSign size={20} /></div>
              <span className="text-xs text-slate-500 uppercase font-bold">Volumen Total (FCT)</span>
           </div>
           <div className="text-3xl font-mono font-bold text-white">{(totalVolume / 1000).toFixed(1)}k</div>
           <div className="text-xs text-slate-400 mt-2">Circulación Global</div>
        </div>

        <div className="bg-[#141417] p-6 rounded-2xl border border-white/5">
           <div className="flex justify-between items-start mb-4">
              <div className="p-3 rounded-lg bg-[#1C8C6E]/10 text-[#1C8C6E]"><TrendingUp size={20} /></div>
              <span className="text-xs text-slate-500 uppercase font-bold">House Profit (GGR)</span>
           </div>
           <div className={`text-3xl font-mono font-bold ${profit >= 0 ? 'text-[#1C8C6E]' : 'text-[#B23A48]'}`}>
        ...
        <div className="bg-[#141417] p-6 rounded-2xl border border-white/5">
           <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs text-slate-500 uppercase font-bold">Sesión Actual</span>
           </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Live Game Feed (Logs) */}
        <div className="lg:col-span-2 bg-[#141417] rounded-2xl border border-white/5 overflow-hidden flex flex-col h-[500px]">
          <div className="p-6 border-b border-white/5 flex justify-between items-center">
            <h3 className="font-heading font-bold text-white">Log de Apuestas en Vivo</h3>
            <div className="relative">
              <Search size={16} className="absolute left-3 top-2.5 text-slate-500"/>
              <input type="text" placeholder="Buscar ID..." className="bg-black/30 border border-white/10 rounded-full pl-9 pr-4 py-2 text-xs text-white focus:border-[#D4C28A] outline-none w-48"/>
            </div>
          </div>
             ...
             <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-[#1a1d21] z-10 shadow-lg">
                   <tr className="text-xs text-slate-400 font-bold uppercase tracking-wider">
                      ...
                      <th className="p-4 text-right">Apuesta</th>
                      <th className="p-4 text-right">Multi</th>
                   ...
                   {logs.length === 0 ? (
                     ...
                     logs.map((log) => (
                       <tr key={log.id} className="border-b border-white/5 hover:bg-white/5 transition-colors animate-fade-in">
                          ...
                          <td className="p-4">
                             <span className="px-2 py-1 rounded bg-slate-800 text-slate-300 text-xs border border-slate-700">{log.game}</span>
                          </td>
                          <td className="p-4 text-right text-slate-300">{log.bet.toFixed(2)}</td>
                          <td className={`p-4 text-right font-bold ${log.multiplier >= 1 ? 'text-[#1C8C6E]' : 'text-slate-500'}`}>
                             {log.multiplier.toFixed(2)}x
                          </td>
                          <td className={`p-4 text-right font-bold ${log.payout > 0 ? 'text-[#1C8C6E]' : 'text-slate-600'}`}>
